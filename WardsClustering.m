...
Title: Wards Hierchial Clustering on PCA Components
Author: William Collins

The purpose of this script is to:
- Read in the ASTER and KLUM data sets.
- Down sample the data to the wavelengths desired
- Perform PCA to reduce dimensionality in the data
- Perform Wards clustering on the all of the data
...
% down sample to data to desired wavelengths
...
maxClusters = 25;  % maximum number of clusters to try
...
excelFilePath = fullfile(path, 'WardsClusterAssignments.xlsx');
...
for k = 6:maxClusters
    for numComponents = 1:maxComponents
        ...
        % Extract clusters using the cluster function
        idx = cluster(Z, 'MaxClust', k);

        % Calculate WCSS for the current number of clusters
        %WCSS(k) = sum(pdist2(reducedData, idx, 'squaredeuclidean'));
