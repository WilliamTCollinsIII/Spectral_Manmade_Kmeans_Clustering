...
% Combine Aster, Klum, and Army
for i = manmadeCount + klumCount + 1 : manmadeCount + klumCount + ArmyCount
    manmadeWavelength{i} = ArmyWavelength(i-manmadeCount - klumCount, :)';
    manmadeReflectivity{i} = ArmyReflectivity(i-manmadeCount - klumCount, :)';
end
manmadeNames = [asterNames; klumNames; ArmyNames];


% Create matricies for data
identity = 1;
[manmadeReflectivityMatrix, manmadeWavelengthMatrix, manmadeIdentityMatrix] = knnMatrixSetUp(manmadeCount + klumCount + ArmyCount, manmadeReflectivity, manmadeWavelength, identity);

% Downsample to wavelengths 
...
group1Keywords = ["paint.solid", "paintarmy"];
group2Keywords = "wood";
group3Keywords = ["metal","brick", "ceramic", "concrete", "limestone", "sandstone", ...
                  "conglomerate", "asphalt", "mortar", "granite"];

% Initialize the nameGroups cell array
nameGroups = cell(3, 1);

% Group files based on the keywords
for i = 1:length(manmadeNames)
    fileName = lower(manmadeNames{i});
    if any(contains(fileName, group1Keywords, 'IgnoreCase', true))
        nameGroups{1} = [nameGroups{1}, i];
    elseif any(contains(fileName, group2Keywords, 'IgnoreCase', true))
        nameGroups{2} = [nameGroups{2}, i];
    elseif any(contains(fileName, group3Keywords, 'IgnoreCase', true))
        nameGroups{3} = [nameGroups{3}, i];
    end
end
